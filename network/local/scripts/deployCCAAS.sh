...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0hospital_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run --rm -d --name peer0patient_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run --rm -d --name peer0pharmacy_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run --rm -d --name peer0insurance_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0hospital_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0patient_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0pharmacy_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0insurance_${CC_NAME}_ccaas  \
                  --network HCNetwork \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
infoln "Installing chaincode on peer0.insurance..."
...
## expect , hospita to have approved and remaining not
checkCommitReadiness hospital
checkCommitReadiness patient
checkCommitReadiness pharmacy
checkCommitReadiness insurance

## approve the definition for patient
approveForMyOrg patient

## check whether the chaincode definition is ready to be committed
## expect , hospital, patien to have approved and remaining not
checkCommitReadiness hospital
checkCommitReadiness patient
checkCommitReadiness pharmacy
checkCommitReadiness insurance
...
## check whether the chaincode definition is ready to be committed
## expect , hospital, patient, pharmac to have approved and remaining not
checkCommitReadiness hospital
checkCommitReadiness patient
checkCommitReadiness pharmacy
checkCommitReadiness insurance
...
## check whether the chaincode definition is ready to be committed
## expect , hospital, patient, pharmacy, insuranc to have approved and remaining not
checkCommitReadiness hospital
checkCommitReadiness patient
checkCommitReadiness pharmacy
checkCommitReadiness insurance
...
commitChaincodeDefinition hospital patient pharmacy insurance
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit hospital patient pharmacy insurance
fi
